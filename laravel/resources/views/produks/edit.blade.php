...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('produks.update', $produk->id) }}" method="POST" class="bg-white p-6 rounded shadow-md">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="stok_awal" id="stok_awal" value="{{ old('stok_awal', $produk->stok_awal) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            @error('stok_awal')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
