    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Isi detail produk dan stok awalnya.</p>
            </div>
        ...
        <form action="{{ route('produks.store') }}" method="POST" class="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div class="grid gap-6 sm:grid-cols-3">
                <div>
                    ...
                    <input id="harga_beli" name="harga_beli" value="{{ old('harga_beli') }}" type="number" min="0" step="0.01" required class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" />
                </div>
                <div>
                    <label for="harga_jual" class="block text-sm font-medium text-slate-700">Harga Jual</label>
                    <input id="harga_jual" name="harga_jual" value="{{ old('harga_jual') }}" type="number" min="0" step="0.01" required class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" />
                </div>
                <div>
                    <label for="stok_awal" class="block text-sm font-medium text-slate-700">Stok Awal</label>
                    <input id="stok_awal" name="stok_awal" value="{{ old('stok_awal') }}" type="number" min="0" required class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" />
                </div>
            </div>

            <div class="flex flex-wrap gap-3">
                <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white hover:bg-sky-700">Simpan Produk</button>
